...
function editTask(id){
 ...
 todoList[id][3] = "shinestatus";
 listMake();
}*/
function deleteTask(id) {
  //id = todoList iin id dah element bolhoor splice(id tah element,heden element arilgah too)
...
function listMake() {
  ...
  if (todoList.length > 0) {
    //todoList dotor bga buh element bolgond creatTodoItem iig duudaj bga
...
function createTask(todoItem) {
  ...
  todoItem.push(todoList.length);
  //todoList deer item iig nemj bga
  todoList.push(todoItem);

  //nemsnii daraa todoList ees buh cardiig shineer uusgej bga
  listMake();
}
